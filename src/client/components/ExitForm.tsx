...
export function Modal({ title, children, onClose, size = 'md' }: { title: string; children: React.ReactNode; onClose: () => void; size?: 'md' | 'lg' | 'xl' | '2xl' }) {
  const maxW = size === '2xl' ? 'max-w-5xl' : size === 'xl' ? 'max-w-3xl' : size === 'lg' ? 'max-w-xl' : 'max-w-md';
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={onClose}>
      <div className={`max-h-[90vh] w-full ${maxW} overflow-y-auto rounded-xl bg-white dark:bg-slate-800 p-5 text-slate-900 dark:text-slate-100`} onClick={(e) => e.stopPropagation()}>
